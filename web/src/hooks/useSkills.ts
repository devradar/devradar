...
export function useSkills() {
    ...
    const loadSkills = useCallback(() => {
        ...
        const skillStats = new Map<string, { count: number; lastPracticed: string; lastLevel: number }>();
    ...
    const addSkill = useCallback((skill: Omit<UserSkill, 'id' | 'created_at' | 'updated_at' | 'is_custom' | 'activity_count' | 'last_practiced'>) => {
        skillsService.addCustomSkill(skill);
        loadSkills();
    }, [loadSkills]);

    const updateSkill = useCallback((id: string, updates: Partial<Omit<UserSkill, 'id' | 'created_at' | 'is_custom'>>) => {
    ...
    const skillsWithActivity = useMemo(() => {
        return skills.filter((s) => (s.activity_count || 0) > 0)
